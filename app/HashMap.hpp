...
public:
    ...
    // hash function cdyou'd like it to use.  A little research online will
...
private:
    ...
    // Helper functions
    void initializeArrayBucket(Node** &newArrayBucket, unsigned int size);

    void copyArrayBucket(Node** target, Node** const &source, unsigned int size);

    void addToTheHead(const std::string& key, const std::string& value, Node* &headPtr);
